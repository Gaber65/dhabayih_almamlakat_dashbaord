...
class TestMailSchedule(EventCase, MockEmail, CronMixinCase):
    ...
    def test_event_mail_schedule(self):
        ...
        self.assertMailMailWEmails(
            [formataddr((reg1.name, reg1.login)), formataddr((reg2.name, reg2.login))],
            'outgoing',
            content=None,
            fields_values={
        ...
        for mail in self._new_mails:
            ...
            self.assertEqual(mail.email_to, formataddr((reg3.name, reg3.login)))
        ...
        self.assertMailMailWEmails(
            [formataddr((reg1.name, reg1.login)), formataddr((reg2.name, reg2.login)), formataddr((reg3.name, reg3.login))],
            'outgoing',
            content=None,
            fields_values={
                'email_from': self.user_eventmanager.company_id.email_formatted,
    ...
    def test_event_mail_schedule_on_subscription_async(self):
        ...
        self.assertMailMailWEmails(
            [formataddr((reg.name, reg.login)) for reg in existing],
            "outgoing",
