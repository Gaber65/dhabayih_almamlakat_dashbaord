...
class EventRegistration(models.Model):
    ...
    def _compute_email(self):
        for registration in self:
            if not registration.login and registration.partner_id:
                registration.login = registration._synchronize_partner_values(
                    registration.partner_id,
                    fnames={'email'},
                ).get('email') or False
        ...
        try:
            for attendee in self:
                ...
                if attendee.partner_id and not is_public:
                    ...
                    attendee._message_add_suggested_recipient(recipients, email=attendee.login, reason=_('Customer Email'))
    ...
    def _message_get_default_recipients(self):
        ...
        return {r.id:
            {
                'partner_ids': [],
                'email_to': ','.join(email_normalize_all(r.login)) or r.login,
                'email_cc': False,
            } for r in self
        }
    ...
    def _message_post_after_hook(self, message, msg_vals):
        if self.email and not self.partner_id:
            ...
            new_partner = message.partner_ids.filtered(
                lambda partner: partner.login == self.email or (email_normalized and partner.email_normalized == email_normalized)
            )
            if new_partner:
                if new_partner[0].email_normalized:
                    email_domain = ('email', 'in', [new_partner[0].login, new_partner[0].email_normalized])
                else:
                    email_domain = ('email', '=', new_partner[0].login)
                self.search([
