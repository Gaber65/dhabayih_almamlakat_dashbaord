...
class TestLivechatBasicFlowHttpCase(HttpCaseWithUserDemo, TestLivechatCommon):
    ...
    def _common_basic_flow(self):
        ...
        self._send_message(channel, self.operator.login, "Message from Operator", author_id=self.operator.partner_id.id)
        ...
        self.assertEqual(channel.message_ids[0].email_from, self.operator.login, "The sender's email should be the operator's email.")
