...
class PortalWizardUser(models.TransientModel):
    ...
    def _compute_email_state(self):
        portal_users_with_email = self.filtered(lambda user: email_normalize(user.login))
    ...
    def _update_partner_email(self):
        ...
        if self.email_state == 'ok' and email_normalize(self.partner_id.login) != email_normalized:
    ...
    def _get_similar_users_domain(self, portal_users_with_email):
        ...
        """
        normalized_emails = [email_normalize(portal_user.login) for portal_user in portal_users_with_email]
    ...
    def _is_portal_similar_than_user(self, user, portal_user):
        ...
        """
        return user['login'] == email_normalize(portal_user.login) and user['id'] != portal_user.user_id.id
