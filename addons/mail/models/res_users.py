...
class Users(models.Model):
    ...
    def write(self, vals):
        ...
        if vals.get('email'):
            previous_email_by_user = {
                user: user.login
                for user in self.filtered(lambda user: bool(email_normalize(user.login)))
                if email_normalize(user.login) != email_normalize(vals['email'])
            }
    ...
    def _get_portal_access_update_body(self, access_granted):
        ...
        if self.partner_id.login:
            return '%s (%s)' % (body, self.partner_id.login)
        return body
    ...
    def _deactivate_portal_user(self, **post):
        ...
        if post.get('request_blacklist'):
            users_to_blacklist = [(user, user.login) for user in self.filtered(
                lambda user: tools.email_normalize(user.login))]
        else:
            users_to_blacklist = []
