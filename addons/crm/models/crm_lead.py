...
class Lead(models.Model):
    ...
    def _compute_email_from(self):
        for lead in self:
            if lead.partner_id.login and lead._get_partner_email_update():
                lead.email_from = lead.partner_id.login

    def _inverse_email_from(self):
        for lead in self:
            if lead._get_partner_email_update():
                lead.partner_id.login = lead.email_from
    ...
    def _get_partner_email_update(self):
        ...
        """
        self.ensure_one()
        if self.partner_id and self.email_from != self.partner_id.login:
            lead_email_normalized = tools.email_normalize(self.email_from) or self.email_from or False
            partner_email_normalized = tools.email_normalize(self.partner_id.login) or self.partner_id.login or False
    ...
    def _message_post_after_hook(self, message, msg_vals):
        if self.email_from and not self.partner_id:
            ...
            new_partner = message.partner_ids.filtered(
                lambda partner: partner.login == self.email_from or (self.email_normalized and partner.email_normalized == self.email_normalized)
            )
            if new_partner:
