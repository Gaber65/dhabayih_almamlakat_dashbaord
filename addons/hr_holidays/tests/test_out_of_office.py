...
class TestOutOfOffice(TestHrHolidaysCommon):
    ...
    def test_leave_ooo(self):
        ...
        partner_info = next(member for member in members_data if member['persona']['email'] == partner.login)
        partner2_info = next(member for member in members_data if member['persona']['email'] == partner2.login)
