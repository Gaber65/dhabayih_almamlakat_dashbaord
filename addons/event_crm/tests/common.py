...
class EventCrmCase(TestCrmCommon, EventCase):
    ...
    def assertLeadConvertion(self, rule, registrations, partner=None, **expected):
        ...
        self.assertEqual(lead.email_from, partner.login if partner and partner.login else registrations._find_first_notnull('email'))
        ...
        for registration in registrations:
            ...
            if registration.login:
                if tools.email_normalize(registration.login) == registration.partner_id.email_normalized:
                    self.assertIn(registration.partner_id.login, lead.description)
                else:
                    self.assertIn(tools.email_normalize(registration.login), lead.description)
