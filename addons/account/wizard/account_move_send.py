...
class AccountMoveSend(models.TransientModel):
    ...
    def _compute_send_mail_extra_fields(self):
        for wizard in self:
            ...
            invoices_without_mail_data = wizard.move_ids.filtered(lambda x: not x.partner_id.login)
    ...
    def action_open_partners_without_email(self, res_ids=None):
        ...
        return self.move_ids.mapped("partner_id").filtered(lambda x: not x.login)._get_records_action(name=_("Partners without email"))
    ...
    def _send_mails(self, moves_data):
        ...
        for move, move_data in [(move, move_data) for move, move_data in moves_data.items() if move.partner_id.login]:
