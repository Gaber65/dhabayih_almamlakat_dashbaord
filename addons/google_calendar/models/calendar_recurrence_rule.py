    ...
    def _write_from_google(self, gevent, vals):
        ...
        for attendee in zip(emails, partners, google_attendees):
            ...
            if email in existing_attendees.mapped('email'):
                ...
                existing_attendees.filtered(lambda att: att.login == email).write({'state': attendee[2].get('responseStatus')})
            else:
        ...
        for odoo_attendee_email in set(existing_attendees.mapped('email')):
            ...
            if email_normalize(odoo_attendee_email) not in emails:
                attendees = existing_attendees.exists().filtered(lambda att: att.login == email_normalize(odoo_attendee_email) and att.partner_id not in organizers_partner_ids)
