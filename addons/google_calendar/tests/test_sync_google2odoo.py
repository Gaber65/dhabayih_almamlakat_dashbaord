...
class TestSyncGoogle2Odoo(TestSyncGoogle):
    ...
    def google_respond_to_recurrent_event_with_option_this_event(self, recurrence, event_index, response_status):
        ...
        updated_event_google_values['attendees'] = [
            {"email": self.organizer_user.partner_id.login, "responseStatus": "accepted"},
            {"email": self.attendee_user.partner_id.login, "responseStatus": response_status},
        ]
        return [
    ...
    def test_new_google_event(self):
        ...
        values = {
            ...
            'attendees': [{
                ...
                'email': self.public_partner.login,
        ...
        admin_attendee = event.attendee_ids.filtered(lambda e: e.login == self.public_partner.login)
        self.assertEqual(self.public_partner.login, admin_attendee.login)
    ...
    def test_new_attendee_and_date_change_does_not_patch_deleted_events(self):
        ...
        google_event = GoogleEvent([{
            ...
            'reminders': {'useDefault': True},
            'attendees': [
                {'email': self.organizer_user.login, 'responseStatus': 'accepted'},
                {'email': self.attendee_user.login, 'responseStatus': 'accepted', 'self': True},
            ],
            'updated': self.now,
            'organizer': {'email': self.organizer_user.login},
        }])
    ...
    def test_recurrence_exdate(self):
        ...
        events = GoogleEvent([{
            'id': recurrence_id,
            'summary': 'Pricing new update',
            'organizer': {'email': self.env.user.login, 'self': True},
            'recurrence': ['RRULE:FREQ=WEEKLY;WKST=SU;COUNT=3;BYDAY=MO'],
            'reminders': {'useDefault': True},
            'start': {'date': '2020-01-6'},
            'end': {'date': '2020-01-7'},
        ...
        with self.mock_datetime_and_now("2024-06-07"):
            ...
            values = [
                # Recurrence from day 7 changes
                {
                    ...
                    "attendees": [
                        {
                            "email": self.organizer_user.partner_id.login,
                            "responseStatus": "accepted",
                        },
                        {
                            "email": self.attendee_user.partner_id.login,
                            "responseStatus": "accepted",
                        },
                    ],
                },
    ...
    def test_several_users_have_the_same_mail(self):
        ...
        user2.partner_id.login = 'test@example.com'
    ...
    def test_private_partner_single_event(self):
        values = {
            ...
            'attendees': [{
                ...
                'email': self.private_partner.login,
                'responseStatus': 'needsAction'
            }, ],
            'reminders': {'useDefault': True},
            'start': {
        ...
        self.env['calendar.event']._sync_google2odoo(GoogleEvent([values]))
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
        private_attendee = event.attendee_ids.filtered(lambda e: e.login == self.private_partner.login)
    ...
    def test_recurrence_private_contact(self):
        ...
        recurrence = self.env['calendar.recurrence'].search([('google_id', '=', values.get('id'))])
        events = recurrence.calendar_event_ids
        private_attendees = events.mapped('attendee_ids').filtered(lambda e: e.login == self.private_partner.login)
    ...
    def test_partner_order(self):
        self.private_partner.login = "internal_user@odoo.com"
        self.private_partner.type = "contact"
        user = self.env['res.users'].create({
            'name': 'Test user Calendar',
            'login': self.private_partner.login,
    ...
    def test_attendee_status_is_not_updated_when_syncing_and_time_data_is_not_changed(self):
        ...
        organizer_state = recurrence.calendar_event_ids.sorted('start')[0].attendee_ids.filtered(lambda attendee: attendee.partner_id.login == organizer.partner_id.login).state
        ...
        values = [{
            ...
            'reminders': {'useDefault': True},
            'organizer': {'email': organizer.partner_id.login},
            'attendees': [{'email': organizer.partner_id.login, 'responseStatus': 'accepted'}, {'email': other_user.partner_id.login, 'responseStatus': 'accepted'}],
            'updated': self.now,
        }]
    ...
    def test_recurring_event_moved_to_future(self, mock_get_events):
        ...
        common = {
            ...
            "organizer": {"email": self.organizer_user.partner_id.login},
            "reminders": {"useDefault": True},
            "summary": "coucou",
