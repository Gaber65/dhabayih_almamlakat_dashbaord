...
class TestSyncOdoo2Google(TestSyncGoogle):
    ...
    def test_event_sync_after_pause_period(self, mock_sync_request):
        ...
        self.assertGoogleEventInserted({
            ...
            'transparency': 'opaque',
            'reminders': {'overrides': [], 'useDefault': False},
            'organizer': {'email': self.organizer_user.login, 'self': True},
            'attendees': [
                            {'email': self.attendee_user.login, 'responseStatus': 'needsAction'},
    ...
    def test_allday_duplicated_first_event_in_recurrence(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'reminders': {'overrides': [], 'useDefault': False},
            'organizer': {'email': self.organizer_user.login, 'self': True},
            'attendees': [{'email': self.organizer_user.login, 'responseStatus': 'accepted'}],
    ...
    def test_sync_odoo2google_multi_company_user_context_mismatch(self):
        ...
        self.assertGoogleEventInsertedMultiTime({
            ...
            'guestsCanModify': True,
            'reminders': {'overrides': [], 'useDefault': False},
            'organizer': {'email': organizer.login, 'self': True},
            'attendees': [{'email': organizer.login, 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_odoo_id' % self.env.cr.dbname: event.id}},
            'transparency': 'opaque',
        })
