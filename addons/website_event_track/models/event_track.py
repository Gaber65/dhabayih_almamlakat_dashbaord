...
class Track(models.Model):
    ...
    def _compute_partner_email(self):
        for track in self:
            if track.partner_id and not track.partner_email:
                track.partner_email = track.partner_id.login
    ...
    def _compute_contact_email(self):
        for track in self:
            if track.partner_id:
                track.contact_email = track.partner_id.login
    ...
    def create(self, vals_list):
        ...
        post_values = {} if self.env.user.login else {'email_from': self.env.company.catchall_formatted}
        for track in tracks:
    ...
    def _message_get_suggested_recipients(self):
        ...
        for track in self:
            if track.partner_id:
                ...
                if track.contact_email and track.contact_email != track.partner_id.login:
                    track._message_add_suggested_recipient(recipients, email=track.contact_email, reason=_('Contact Email'))
                if not track.contact_email and track.partner_email and track.partner_email != track.partner_id.login:
    ...
    def _message_post_after_hook(self, message, msg_vals):
        ...
        if msg_vals.get('partner_ids') and not self.partner_id:
            ...
            new_partner = message.partner_ids.filtered(
                lambda partner: partner.login == main_email or (main_email_normalized and partner.email_normalized == main_email_normalized)
            )
            if new_partner:
                mail_email_fname = 'contact_email' if self.contact_email else 'partner_email'
                if new_partner[0].email_normalized:
                    email_domain = (mail_email_fname, 'in', [new_partner[0].login, new_partner[0].email_normalized])
                else:
                    email_domain = (mail_email_fname, '=', new_partner[0].login)
                self.search([
