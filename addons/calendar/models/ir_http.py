    ...
    def _auth_method_calendar(cls):
        ...
        if not attendee:
            ...
            if attendee.partner_id != user.partner_id:
                error_message = """Invitation cannot be forwarded via email. This event/meeting belongs to %s and you are logged in as %s. Please ask organizer to add you.""" % (attendee.login, user.login)
        if error_message:
