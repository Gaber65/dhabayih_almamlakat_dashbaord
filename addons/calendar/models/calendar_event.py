    ...
    def _compute_invalid_email_partner_ids(self):
        for event in self:
            event.invalid_email_partner_ids = event.partner_ids.filtered(
                lambda a: not (a.login and single_email_re.match(a.login))
            )
    ...
    def _get_ics_file(self):
        ...
        for meeting in self:
            ...
            for attendee in meeting.attendee_ids:
                ...
                attendee_add.value = u'MAILTO:' + (attendee.login or u'')

            # Add "organizer" field if email available
            if meeting.partner_id.login:
                organizer = event.add('organizer')
                organizer.value = u'MAILTO:' + meeting.partner_id.login
