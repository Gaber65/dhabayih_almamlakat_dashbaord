...
class TestMailGroup(TestMailListCommon):
    ...
    def test_find_group_user_for_alias(self):
        ...
        err_msg = self.test_group._alias_get_error({}, {'email_from': group_user_not_member.login}, self.test_group.alias_id)
    ...
    def test_find_member_for_alias(self):
        ...
        user2 = mail_new_test_user(self.env, login='login_2', email=user.login)
        ...
        self.assertEqual(member.login, user.login)
    ...
    def test_join_group(self):
        ...
        self.assertEqual(member.login, portal_partner.login, 'Should have force the email to the email of the partner')
        self.assertEqual(member.email_normalized, portal_partner.email_normalized)

        portal_partner.login = 'new_portal_email@example.com'
        self.assertEqual(member.login, 'new_portal_email@example.com', 'Should have change the email of the partner')
    ...
    def test_mail_group_member_security(self):
        ...
        self.assertEqual(member.login, '"Member 1" <member_1@test.com>', msg='Moderators should have access to members')
