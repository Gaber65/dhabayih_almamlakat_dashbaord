...
class TestMailGroupMessage(TestMailListCommon):
    ...
    def test_batch_send(self):
        ...
        with self.mock_mail_gateway():
            self.format_and_process(
                GROUP_TEMPLATE, self.test_group.member_ids[0].login,
    ...
    def test_email_not_sent_to_author(self):
        ...
        with self.mock_mail_gateway():
            self.format_and_process(
                GROUP_TEMPLATE, self.test_group_member_1.login,
                self.test_group.alias_id.display_name,
        ...
        self.assertNotIn(self.test_group_member_1.login, mails.mapped('email_to'), 'Should not have send the email to the original author')
