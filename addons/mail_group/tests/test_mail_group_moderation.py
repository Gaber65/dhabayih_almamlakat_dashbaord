...
class TestMailGroupModeration(TestMailListCommon):
    ...
    def test_moderation_rule_api(self):
        ...
        new_moderation = mail_group.moderation_rule_ids.filtered(lambda rule: rule.login == 'bob@test.com')
    ...
    def test_moderation_rule_email_normalize(self):
        ...
        self.assertEqual(rule.login, 'bob@test.com')

        rule.login = '"Alice" <alice@test.com>'
        self.assertEqual(rule.login, 'alice@test.com')
    ...
    def test_moderation_rule_security(self):
        with self.assertRaises(AccessError, msg='Portal should not have access to moderation rules'):
            self.env['mail.group.moderation'].with_user(self.user_portal).browse(self.moderation.ids).login
        ...
        with self.assertRaises(AccessError, msg='Non moderators should not have access to moderation rules'):
            self.env['mail.group.moderation'].with_user(self.user_employee).browse(self.moderation.ids).login

        self.assertEqual(
            self.env['mail.group.moderation'].with_user(self.user_admin).browse(self.moderation.ids).login,
...
class TestModeration(TestMailListCommon):
    ...
    def setUpClass(cls):
        ...
        cls.test_group_2_member_emp = cls.env['mail.group.member'].create({
            ...
            'email': cls.user_employee_2.login,
