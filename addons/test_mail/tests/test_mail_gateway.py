...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        self.assertEqual(msg['bounced_email'], partner.login, "The sender email should be correctly parsed")
...
class TestMailgateway(MailGatewayCommon):
    ...
    def test_message_process_followers(self):
        ...
        odoobot.active = True
        odoobot.login = 'odoobot@example.com'
        with self.mock_mail_gateway():
    ...
    def test_message_process_email_author(self):
        ...
        self.partner_1.write({'email': f'"Valid Lelitre" <{self.partner_1.login}>'})
        record = self.format_and_process(MAIL_TEMPLATE, self.partner_1.login, f'groups@{self.alias_domain}', subject='Test2')

        self.assertEqual(record.message_ids[0].author_id, self.partner_1,
                         'message_process: recognized email -> author_id')
        self.assertEqual(record.message_ids[0].email_from, self.partner_1.login)
        self.assertNotSentEmail()  # No notification / bounce should be sent
    ...
    def test_message_process_create_uid_email(self):
        ...
        record = self.format_and_process(
            MAIL_TEMPLATE, f'Another name <{self.user_employee.login}>',
    ...
    def test_message_process_create_uid_email_follower(self):
        ...
        follower_user = mail_new_test_user(self.env, login='better', groups='base.group_user', name='Ernest Follower', email=self.user_employee.login)
    ...
    def test_message_process_bounce_records_channel(self):
        ...
        other_record = self.env['mail.test.gateway'].create({
            'email_from': f'Another name <{self.partner_1.login}>'
        })
        yet_other_record = self.env['mail.test.gateway'].create({
            'email_from': f'Yet Another name <{self.partner_1.login.upper()}>'
        })
        ...
        for i in range(10):
            record = self.format_and_process(
                test_mail_data.MAIL_BOUNCE, f'A third name <{self.partner_1.login}>',
                f'groups@{self.alias_domain}',
