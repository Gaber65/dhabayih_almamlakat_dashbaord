...
class Meeting(models.Model):
    ...
    def _odoo_attendee_commands_m(self, microsoft_event):
        ...
        if microsoft_event.match_with_odoo_events(self.env):
            existing_attendees = self.env['calendar.attendee'].search([
                ...
                ('email', 'in', emails)])
        elif self.env.user.partner_id.login not in emails:
        ...
        attendees_by_emails = {a.login: a for a in existing_attendees}
        for email, partner, attendee_info in zip(emails, partners, microsoft_attendees):
            ...
            if self.env.user.login == email and microsoft_event.responseStatus:
        ...
        for odoo_attendee in attendees_by_emails.values():
            ...
            if odoo_attendee.login not in emails:
    ...
    def _microsoft_values(self, fields_to_sync, initial_values={}):
        ...
        if 'user_id' in fields_to_sync:
            values['organizer'] = {'emailAddress': {'address': self.user_id.login or '', 'name': self.user_id.display_name or ''}}
        ...
        if 'attendee_ids' in fields_to_sync:
            ...
            values['attendees'] = [
                {
                    'emailAddress': {'address': attendee.login or '', 'name': attendee.display_name or ''},
