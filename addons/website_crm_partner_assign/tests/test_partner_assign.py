...
class TestPartnerLeadPortal(TestCrmCommon, HttpCase):
    ...
    def test_lead_update(self):
        ...
        self.assertEqual(test_partner.login, email_1, 'Adress email on the partner should not be updated')

        test_user.unlink()
        opportunity.with_user(self.user_portal).update_contact_details_from_portal({
            'email_from': email_2,
        })
        self.assertEqual(test_partner.login, email_2, 'Adress email on the partner must be updated')
