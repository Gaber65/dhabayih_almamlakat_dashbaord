...
class Applicant(models.Model):
    ...
    def _compute_partner_phone_email(self):
        for applicant in self:
            if not applicant.partner_id:
                continue
            applicant.email_from = applicant.partner_id.login
    ...
    def _inverse_partner_email(self):
        for applicant in self:
            ...
            if tools.email_normalize(applicant.email_from) != tools.email_normalize(applicant.partner_id.login):
                ...
                applicant.partner_id.login = applicant.email_from
...
<p>%(para_1)s<br/>%(para_2)s</p>""") % {
    ...
    def _message_post_after_hook(self, message, msg_vals):
        if self.email_from and not self.partner_id:
            ...
            new_partner = message.partner_ids.filtered(
                lambda partner: partner.login == self.email_from or (email_normalized and partner.email_normalized == email_normalized)
            )
            if new_partner:
                ...
                if new_partner[0].email_normalized:
                    email_domain = ('email_from', 'in', [new_partner[0].login, new_partner[0].email_normalized])
                else:
                    email_domain = ('email_from', '=', new_partner[0].login)
                self.search([
    ...
    def _get_employee_create_vals(self):
        ...
        return {
            ...
            'private_email': address_sudo.login,
            'lang': address_sudo.lang,
            'department_id': self.department_id.id,
            'address_id': self.company_id.partner_id.id,
            'work_email': self.department_id.company_id.login or self.email_from, # To have a valid email address by default
