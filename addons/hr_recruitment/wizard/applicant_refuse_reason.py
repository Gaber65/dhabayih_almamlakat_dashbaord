...
class ApplicantGetRefuseReason(models.TransientModel):
    ...
    def _compute_applicant_without_email(self):
        for wizard in self:
            applicants = wizard.applicant_ids.filtered(lambda x: not x.email_from and not x.partner_id.login)
    ...
    def action_refuse_reason_apply(self):
        if self.send_mail:
            ...
            if not self.applicant_ids.filtered(lambda x: x.email_from or x.partner_id.login):
        ...
        if self.send_mail:
            applicants = self.applicant_ids.filtered(lambda x: x.email_from or x.partner_id.login)
